  ...
  onUpload(doc=document) {
    ...
    ulButton.onclick = () => {
      ...
      fileReader.onload = (e) => {
        ...
        this.controller.localInsert(fileText, 0);
        this.controller.crdt.populateText();
        this.replaceText(this.controller.crdt.text);
        this.hideUpload();
      }
...
//          this.processInsert(changeObj);    // uncomment this line for palindromes!
  ...
  processInsert(changeObj) {
    ...
    this.controller.localInsert(chars, changeObj.from);
  }
  ...
  insertText(value, positions, siteId) {
    ...
    if (localCursor.line === positions.to.line && localCursor.ch >= positions.to.ch) {
      localCursor.ch++;
    }
  ...
  updateRemoteCursorsInsert(position, siteId) {
    for (const cursorSiteId in this.remoteCursors) {
      ...
      if (newPosition.line === position.line && newPosition.ch >= position.ch) {
        newPosition.ch++;
      }
  ...
  updateRemoteCursorsDelete(position, siteId) {
    for (const cursorSiteId in this.remoteCursors) {
      if (cursorSiteId === siteId) continue;
      const remoteCursor = this.remoteCursors[cursorSiteId];
      const newPosition = Object.assign({}, remoteCursor.lastPosition);
