...
//          this.processInsert(changeObj);    // uncomment this line for palindromes!
  ...
  insertText(values, positions, siteId) {
    const localCursor = this.mde.codemirror.getCursor();
    const delta = this.generateDeltaFromChars(values);

    this.mde.codemirror.replaceRange(values, positions.from, positions.from, 'insertText');
    this.updateRemoteCursorsInsert(positions.to, siteId);
    this.updateRemoteCursor(positions.to, siteId, 'insert', values);
  ...
  updateRemoteCursorsInsert(chars, position, siteId) {
    ...
    for (const cursorSiteId in this.remoteCursors) {
      ...
      const lastPosition = Object.assign({}, remoteCursor.lastPosition);

      if (lastPosition.line > position.line) {
        lastPosition.line += positionDelta.line;
      } else if (lastPosition.line === position.line && lastPosition.ch > position.ch) {
        if (positionDelta.line > 0) {
          lastPosition.line += positionDelta.line;
          lastPosition.ch -= position.ch;
        }

        lastPosition.ch += positionDelta.ch;
      }

      remoteCursor.set(lastPosition)
    }
  }
  ...
  updateRemoteCursorsDelete(chars, to, from, siteId) {
    ...
    for (const cursorSiteId in this.remoteCursors) {
      ...
      if (lastPosition.line > to.line) {
        lastPosition.line -= positionDelta.line;
      } else if (lastPosition.line === to.line && lastPosition.ch > to.ch) {
        if (positionDelta.line > 0) {
          lastPosition.line -= positionDelta.line;
          lastPosition.ch += from.ch;
        }
  ...
  deleteText(values, positions, siteId) {
    const localCursor = this.mde.codemirror.getCursor();
    const delta = this.generateDeltaFromChars(values);
