...
class Editor {
  ...
  bindChangeEvent() {
    this.mde.codemirror.on("change", (_, changeObj) => {
      ...
      if (changeObj.origin === "+input") {
        ...
        changedChar = this.controller.crdt.handleLocalInsert(char, idx);
      } else if (changeObj.origin === "+delete") {
        changedChar = this.controller.crdt.handleLocalDelete(idx);
      }

      this.controller.conns.forEach(conn => {
  ...
  updateView() {
    ...
    this.mde.value(this.controller.crdt.text);
