...
// when are the above two lines true?
  ...
  processInsert(changeObj) {
    ...
    let charPos = this.findCharPosition(changeObj);
    let idx;

    chars.forEach(lineOfChars => {
      idx = this.findLinearIdx(linePos, charPos);
      this.controller.handleInsert(lineOfChars, idx);
      linePos++;
      charPos = 0;
    });
  }
  ...
  findCharPosition(changeObj) {
    if (changeObj.text[0][0] === '\t') {  // so this is true for spaces too right?
