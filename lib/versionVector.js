...
class VersionVector {
  ...
  constructor(controller) {
    this.controller = controller; // keep the same
    this.versions = new SortedArray(this.siteIdComparator); // replace completely
    this.localVersion = new Version(controller["siteId"]); // replace localVersion.counter and .exceptions
    this.versions.insert(this.localVersion); // this needs to be done manually
  }
  ...
  hasBeenApplied(incomingVersion) {
    ...
    const index = this.versions.indexOf(incomingVersion);

//    if (index < 0) return false;

    const localVersion = this.versions.get(index);
    const incomingExistsInVersionVector = !!localVersion;

    if (!incomingExistsInVersionVector) return false;

    const incomingIsLower = incomingVersion.counter <= localVersion.counter;
    const isInExceptions = localVersion.exceptions.has(incomingVersion.counter);

    return incomingIsLower && !isInExceptions;
  }
