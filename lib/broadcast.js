class Broadcast {
  ...
  connectPeers(peerId, siteId, newPeerZone){
    ...
    if(origNeighbors.length !== 0){
      this.neighbors.forEach((neighbor) => {                                  //for each neighbor of original zone,
        const connNeighbor =  origNeighbors.find(conn => neighbor.peerId === conn.peer); // neighbor conn object
        if(connNeighbor){
  ...
  // Connecting peer accepts neighbor passed to it by original peer
