class Broadcast {
  ...
  connectNeighbor(peerId, siteId, newPeerZone){
    const connected = this.outConns.filter(conn => conn.peer !== peerId); 
    connected.forEach((conn) => {
      if(isNeighbor(conn, newPeerZone)){
        conn.send(JSON.stringify({
          type: 'addNeighbor',
          peerId: peerId,
          siteId: siteId,
        }));
        console.log('Neighbor zone: ' + newPeerZone);
      }
    });
  ...
  isNeighbor(connection, newPeerZone){
    ...
    console.log('===CONNECTION REGION: ' + connection.broadcast.region);
  ...
  acceptNeighbor(peerId, siteId) {
    console.log("==================ACCEPT NEIGHBOR RUNNING ======")
  ...
  acceptConnRequest(peerId, siteId) {
    ...
    this.connectNeighbor(peerId, siteId, zoneOffer)
  }
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'addNeighbor':                                    // adding neighbor to original peer
          this.acceptNeighbor(dataObj.peerId, dataObj.siteId); // similar to acceptConnRequest() w/o zone sharing
          break;
