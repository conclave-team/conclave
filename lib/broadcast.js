class Broadcast {
  ...
  zoneTake(xStart,xEnd,yStart,yEnd){ //split zone by new peer
    console.log("Zone Take Running");

    if((yEnd-yStart)>(xEnd-xStart)){
      this.region = [[xStart,xEnd],[yStart,((yEnd+yStart)/2)]];
    }else{
      this.region = [[xStart,(xEnd+xStart)/2],[yStart,yEnd]];
    }
    this.refPoint = [Math.floor((Math.random()*(region[0][1]-region[0][0]))+region[0][0]), Math.floor((Math.random()*(region[1][1]-region[1][0]))+region[1][0])];

    console.log("New zone of connecting peer: ${this.region}");
  }

  zoneShare(){ //split zone by original owner
    console.log("Zone Share Running");

    let xStart = region[0][0];
    let xEnd = region[0][1];
    let yStart = region[1][0];
    let yEnd = region[1][1];
    ...
    console.log("New zone of OG peer: ${this.region}");
  }
  ...
  acceptConnRequest(peerId, siteId) {
    ...
    const initialData = JSON.stringify({
      ...
      //additions
      xStart: this.region[0][0],
      xEnd: this.region[0][1],
      yStart: this.region[1][0],
      yEnd: this.region[1][1],
    });
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'syncResponse':
          ...
          this.zoneTake(dataObj.xStart,dataObj.xEnd,dataObj.yStart,dataObj.yEnd);
          break;
        case 'syncCompleted':
          this.processOutgoingBuffer(dataObj.peerId);
          break;
