class Broadcast {
  ...
  processOutgoingBuffer(peerId) {
    ...
    this.redistribute(connection);
  }

  redistribute(conn) {
    const halfTheNetwork = Math.ceil(this.controller.network.length / 2);
    if (this.connections.length > Math.max(halfTheNetwork, 5)) {
      conn.close();
      this.connections = this.connections.filter(c => c.peer !== conn.peer);
      this.controller.addToNetwork(this.peer.id, this.controller.siteId);
    }
  }
  ...
  //     this.peer.reconnect();
  //     console.log("I think I am reconnected now.");
  //   });
  //   this.peer.on('close', () => this.peer.destroy());
  // }
  ...
  onConnection(connection) {
    connection.on('open', () => {
      ...
      const halfTheNetwork = Math.ceil(this.controller.network.length / 2);
      if (this.connections.length > Math.max(halfTheNetwork, 5)) {
        const myNetwork = this.controller.network.filter(obj => obj.peerId !== this.peer.id);
        const randomIdx = Math.floor(Math.random() * myNetwork.length);
        const newId = myNetwork[randomIdx].peerId;
        connection.send(JSON.stringify({type: 'redirect', newId: newId}));
      } else {
  ...
  onData(connection) {
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        case 'redirect':
          this.connectToNewTarget(dataObj.peerId);
          break;
        case 'syncResponse':
