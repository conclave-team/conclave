class Broadcast {
  ...
  findTakeOver(departedGenValue,departedRegion,peerId,siteId,peerZone,genValue,visited){
    ...
    // let departedPredecessor = this.departedGenValue.slice(0,this.genValue.length-1);
    // let bitDistance = this.checkBitDistance(departedGenValue, this.genValue);
    let lowestDistance = null;
    let predecessorNeighbor = null;
    let connection = null;
    let x = 0;
    let visitedList = visited.split(',');
    
    // Check if I am the takeover node
    if(this.genValue === departedGenValue) {
      // send back to addNeighbor
      ...
      this.predecessorNeighbor.forEach(neighbor => {
      ...
      visitedList.push(this.genValue);
  ...
  refreshTimer(peerId){
    ...
    let takeOver = (idx) => {
      ...
      let departedGenValue = this.neighbors[idx].genValue;
      ...
      if(this.isTakeOver(this.neighbors[idx].genValue, this.genValue)){
        ...
        //if sibling
        let bitDistance = this.checkBitDistance(departedGenValue, this.genValue)
        if(this.isPredecessor(this.genValue, departedPredecessor) && (bitDistance === 1)){
          console.log('\tDEBUG": \t I AM SIBLINGS WITH DEPARTED');
          console.log('\tDEBUG": \t departed region ' + this.neighbors[idx].region);
          console.log('\tDEBUG": \t departed prede ' + departedPredecessor);
          
          this.zoneMerge(this.neighbors[idx].region, departedPredecessor);
        }
        else{
          console.log('\tDEBUG: \t SIBLINGS = FALSE');
          this.zoneTransfer(this.neighbors[idx].region,departedGenValue);
          //send 'zoneleave'
        }
      }else{
        ...
        this.findTakeOver(this.neighbors[idx].genValue,this.neighbors[idx].region , this.peer.id, this.controller.siteId, this.region, this.genValue, this.genValue);
  ...
  zoneTransfer(departedZone, departedGenValue){
    ...
    console.log('\tDEBUG: \tSIBLING: ' + Object.values(sibling));
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'findTakeOver':
          ...
          this.findTakeOver(dataObj.departedGenValue,dataObj.peerId,dataObj.siteId,dataObj.region,dataObj.genValue,dataObj.visited);
        case 'takeOver':
          this.delay(20000);
