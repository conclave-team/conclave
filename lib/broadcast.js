class Broadcast {
  ...
  send(operation) {
    ...
    this.outConns.forEach(conn => {
      if (conn.open) conn.send(operationJSON);
    });
  }
  ...
  evaluateSecondary(peerId, siteId) {
    if (this.hasReachedMax()) {
      this.forwardSecondaryRequest(peerId, siteId);
    } else {
      this.acceptSecondaryRequest(peerId, siteId);
    }
  }

  hasReachedMax() {
    const halfTheNetwork = Math.log(this.controller.network.length) + 2;
  ...
  acceptSecondaryRequest(peerId, siteId) {
  ...
  connectTo(network) {
    const connections = network.slice(network.length - 3, network.length - 1)
                               .filter(conn => conn.peerId !== this.peer.id);
    connections.forEach(conn => {
      this.requestSecondaryConnection(conn.peerId, this.peer.id, this.controller.siteId);
    });
  }
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'syncResponse':
          ...
          this.connectTo(dataObj.network);
