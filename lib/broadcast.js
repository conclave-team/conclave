class Broadcast {
  ...
  onVideoCall() {
    this.peer.on('call', callObj => {
      const peerLi = document.getElementById(callObj.peer);
      const peerFlag = peerLi.children[0];
      const color = peerFlag.style.backgroundColor;

      this.controller.lightOn(peerLi);

      navigator.mediaDevices.getUserMedia({audio: true, video: true})
      .then(ms => {
        peerFlag.onclick = () => {
          callObj.answer(ms);
          this.controller.lightOff(peerLi);
          this.onStream(callObj, color);
        };
      });
    });
  ...
  onStreamClose(vid, peerId, doc=document) {
    vid.style.visibility = 'hidden';
    this.currentStream.localStream.getTracks().forEach(track => track.stop());
    this.currentStream = null;

    const peerFlag = doc.getElementById(peerId).children[0];
    peerFlag.onclick = () => {
      navigator.mediaDevices.getUserMedia({audio: true, video: true})
