class Broadcast {
  ...
  refreshTimer(peerId){
    ...
    const takeOver = () => {
      ...
      if(this.compareGenValue(this.neighbors[idx].genValue, this.genValue)){
        
      }

    }

    console.log('\tREFRESH:\t----NEIGHBOR: ' + peerId);
    ...
    if(this.neighbors[idx].timer === null){
      //console.log('\tDEBUG:\t'+ this.neighbors[idx].timer);
      this.neighbors[idx].timer = setTimeout(function() { takeOver(); }, 50000);
    }else{
      //console.log('\tDEBUG:\t'+ this.neighbors[idx].timer);
      clearTimeout(this.neighbors[idx].timer);
      //console.log('\tDEBUG:\t'+ this.neighbors[idx].timer);
  ...
  compareGenValue(departedGenValue, myGenValue){
    // compare departed node's genval with this genval

    // case 1: departing node has sibling
    if(departedGenValue.length === myGenValue.length){
      
    }
    // case 2: departing node has no sibling
    else{
