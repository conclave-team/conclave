class Broadcast {
  ...
  isNeighbor(connection){

    //let xEnd1 = connection.broadcast.region[][];
    //let xStart1 = connection.broadcast.region[][];
    //let xEnd2 = connection.broadcast.region[][];
    //let xStart2 = connection.broadcast.region[][];

    //let yEnd1 = connection.broadcast.region[][];
    //let yStart1 = connection.broadcast.region[][];
    //let yEnd2 = connection.broadcast.region[][];
    //let yStart2 = connection.broadcast.region[][];
  ...
  zoneTake(xStart,xEnd,yStart,yEnd,Gen) { //split zone by new peer
    console.log("Zone Take Running");


    this.region = [[xStart,xEnd],[yStart,yEnd]];
    ...
    console.log("=======ZONE OF THIS PEER: " + this.region);
    console.log("=======GENVAL: " + this.genValue);
  }

  zoneShare() { //split zone by original owner
    console.log("Zone Share Running");
    ...
    if((yEnd-yStart)>(xEnd-xStart)){
      zoneOffer = [[xStart,xEnd],[yStart,((yEnd+yStart)/2)]];
    }else{
      zoneOffer = [[xStart,(xEnd+xStart)/2],[yStart,yEnd]];
    }
  ...
  acceptConnRequest(peerId, siteId) {
    ...
    const initialData = JSON.stringify({
      ...
      //additions
      xStart: zoneOffer[0][0],
      xEnd: zoneOffer[0][1],
      yStart: zoneOffer[1][0],
      yEnd: zoneOffer[1][1],
      Gen: this.genValue,
    });
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'syncResponse':
          ...
          this.zoneTake(dataObj.xStart,dataObj.xEnd,dataObj.yStart,dataObj.yEnd,dataObj.Gen);
          break;
        case 'syncCompleted':
          this.processOutgoingBuffer(dataObj.peerId);
          break;
