class Broadcast {
  ...
  regionRevert(region){
    let zone = [[region[0], region[1]],[region[2], region[3]]];
    return zone;
  ...
  // For passing neighbors from original peer to connecting peer
    ...
    if(origNeighbors.length !== 0){
      this.neighbors.forEach((neighbor) => {                                  //for each neighbor of original zone,
        ...
        if(connNeighbor){
          ...
          if(this.isNeighbor(this.regionRevert(neighbor.region), this.regionRevert(newPeerZone))){
  ...
  updateNeighbor(peerId, siteId, region){ 
    ...
    this.neighbors.forEach((neighbor) => {
      if(peerId === neighbor.peerId){
        console.log('\tDEBUG:\t'+ this.regionAdapter(this.region) + '\t' + typeof this.regionAdapter(this.region));
        console.log('\tDEBUG:\t'+ this.regionAdapter(region) + '\t' + typeof region);
  ...
  isNeighbor(neighborZone, newPeerZone){
    neighborZone = this.regionAdapter(neighborZone);
    newPeerZone = this.regionAdapter(newPeerZone);
    console.log('\tDEBUG:\tIS NEIGHBOR SUCCESS');
    let neighbor = null;
  ...
  // Neighbor Peer of original peer accepts connecting peer as neighbor
  ...
  acceptNeighbor(peerId, siteId, newPeerZone) {
    ...
    const initialData = JSON.stringify({
      ...
      //additions
      peerZone: this.regionAdapter(this.region),
    });
  ...
  zoneShare() { //split zone by original owner
    ...
    zoneOffer[0] = this.regionAdapter(Offer);
    ...
    zoneOffer[1] = this.regionAdapter(this.region);
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'askUpdateNeighbor':
          ...
          this.replyRequest(dataObj.peerId); // adds 
          break;
