class Broadcast {
  ...
  findTakeOver(departedGenValue,peerId,siteId,peerZone,genValue){
    let initialDistance = this.checkBitDistance(departedGenValue, this.genValue);
    let lowestDistance = initialDistance;
    let connection = null;
    this.neighbors.forEach(neighbor => {
      let currDistance = this.checkBitDistance(departedGenValue, neighbor.genValue);
      if (neighbor.genValue === departedGenValue) {
        console.log('departedGenValue ' + departedGenValue + '===' + 'NeighborGenValue' + neighbor.genValue);
        return;
      }
    ...
    if (lowestDistance<initialDistance) { // there is a neighbor that is nearer to the departed node
      console.log('\tDEBUG: \t findTakeOver forwarded');
      connection.send(JSON.stringify({
        type: 'findTakeOver',
        peerGenValue: departedGenValue,
        peerId: peerId,
        siteId: siteId,
        region: peerZone,
        genValue: genValue,
  ...
  refreshTimer(peerId){
    ...
    let takeOver = (idx) => {
      ...
      if(this.isTakeOver(this.neighbors[idx].genValue, this.genValue)){
        
        //if sibling
        if(this.neighbors[idx].genValue.length === this.genValue.length){
          console.log('\tDEBUG": \t SIBLINGS = TRUE!');
          // zoneMerge() ?
        }
        else{
          //zoneTransfer()
        ...
        console.log('\tDEBUG": \t I AM THE TAKEOVER NODE');
      }
      else{
        ...
        this.findTakeOver(this.neighbors[idx].genValue, this.peer.id, this.controller.siteId, this.region, this.genValue);
  ...
  zoneMerge(){ // params: some way to reference disconnecting and merging z
    ...
    let temp1 = departingZone.split(',').map(Number); // zone of disconnecting
  ...
  isTakeOver(departedGenValue, myGenValue){
    ...
    if (departedGenValue.length !== 1){
      ...
      let myTempGenVal = myGenValue.slice(0,departedPredecessor.length);
      console.log('\tDEBUG: departedPredecessor\t'+ departedPredecessor);
      console.log('\tDEBUG: myTempGenVal\t'+ myTempGenVal);
      if(departedPredecessor === myTempGenVal){ //filters to only the neighbors from the same predecessor subtree
        //make sure both departedGenValue and myGenValue have the same length
  ...
  checkBitDistance(neighborGenVal, myGenValue){
    ...
    if(myGenValue.length>neighborGenVal.length){
      ...
      while(myGenValue.length !== tempDeparted.length){
        myGenValue = myGenValue + "0";
      }
    }
    
    //check if one bitwise distance away
    for(let i = 0;i<myGenValue.length;i++){
      if(tempDeparted[i] !== myGenValue[i]){
        bitDistance++;
      }
    }
  ...
  isNeighbor(neighborZone, newPeerZone) {
    let intersections = this.getIntersection(neighborZone, newPeerZone);
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'findTakeOver':
          ...
          this.findTakeOver(dataObj.peerGenValue,dataObj.peerId,dataObj.siteId,dataObj.region,dataObj.genValue);
        default:
