...
class Controller {
  ...
  handleRemoteOperation(data) {
    ...
    if (this.vector.isDuplicate(dataObj.version)) return;
  ...
  reviewBuffer() {
    ...
    for(let i = this.buffer.length - 1; i >= 0; i--) {
      ...
      if (this.vector.isDuplicate(dataObj.version)) {
...
new Controller(gPEERID, gHOST, new Peer({key: 'mgk9l45fu1gfzuxr', debug: 1}));
