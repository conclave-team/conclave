...
class Controller {
  ...
  createNewUrl(id=this.randomId(), doc=document) {
    this.refreshId = id;
  ...
  randomId() {
    const possConns = this.broadcast.inConns.filter(conn => {
      return this.broadcast.peer.id !== conn.peer;
    });
...
console.log("possible targets inside findNewTarget", possibleTargets);
    if (possibleTargets.length === 0) {
      this.removeUrl();
      this.broadcast.peer.on('connection', conn => {
        console.log("in findNewTarget, no possible targets, in peer.on connection");
  ...
  handleSync(syncObj, doc=document) {
    console.log("in handle sync, i am connected to ", syncObj.peerId);
    if (syncObj.peerId != this.urlId) {
      console.log("in handle sync, syncObj peerId different than url id");
