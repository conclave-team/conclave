...
class Controller {
  ...
  insertIntoEditor(value, index) {
    const position = this.findEditorPosition(index);
    this.editor.insertText(value, position);
  }

  deleteFromEditor(index) {
    this.editor.deleteText(index);
  }

  findEditorPosition(index) {
    ...
    const numLastLineChars = substring.slice(lastNewlineIdx + 1).length;

    return {
      line: numNewlines,
      ch: numLastLineChars - 1,
    }
  }
}
