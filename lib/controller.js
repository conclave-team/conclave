...
class Controller {
  ...
  insertIntoEditor(chars, pos, siteId) {
    const positions = {
      from: pos,
      to: pos
    }

    this.editor.insertText(chars.map(char => char.value), positions, siteId);
  }
  ...
  deleteFromEditor(chars, startPos, endPos, siteId) {
    ...
    this.editor.deleteText(chars.map(char => char.value), positions, siteId);
  }
}
