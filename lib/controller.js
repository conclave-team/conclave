...
class Controller {
  ...
  populateCRDT(initialStructData) {
    const struct = JSON.parse(initialStructData).map(ch => {
  ...
  populateVersionVector(initialVersionsData) {
    const versions = JSON.parse(initialVersionsData).arr.map(ver => {
  ...
  addToConnectionList(id, doc=document) {
  ...
  removeFromConnectionList(id, doc=document) {
    doc.getElementById(id).remove();
  }

  handleSync(syncObj) {
    this.populateCRDT(syncObj.initialStruct);
    this.populateVersions(syncObj.initialVersions);
  }
