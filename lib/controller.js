...
class Controller {
  constructor(targetPeerId, host, peer, zone, broadcast, editor, doc=document, win=window) {
    this.siteId = UUID();
    this.host = host;
    this.buffer = [];
    this.calling = [];
    ...
    if (targetPeerId == 0) {
      // Zone of first peer = the whole cartesian space
      cartesianSpace = [[0,10],[0,10]]
      this.broadcast.region = cartesianSpace;
      this.broadcast.refPoint = [Math.floor((Math.random()*(cartesianSpace[0][1]-cartesianSpace[0][0]))+cartesianSpace[0][0]), Math.floor((Math.random()*(cartesianSpace[1][1]-cartesianSpace[1][0]))+cartesianSpace[1][0])]
      this.broadcast.genValue = 1;
      console.log("Cartesian Space: " + this.region);
    }
