...
class Controller {
  ...
  handleRemoteOperation(operation) {
    let op;
    if (this.vector.hasBeenApplied(operation.versions[0])) return;

    if (operation.type === 'insert') {
      operation.chars.forEach((char, i) => {
        op = {
          type: 'insert',
          char: char,
          version: operation.versions[i]
        }
  ...
  broadcastInsertion(chars) {
    while (chars.length > 0) {
      ...
      const operation = {
        type: 'insert',
        chars: someChars,
        versions: someChars.map(char => ({siteId: char.siteId, counter: char.counter}))
      };
  ...
  broadcastDeletion(chars, versions) {
    while (chars.length > 0) {
      let someChars = chars.splice(0, 100);
      let someVersions = versions.splice(0, 100);
