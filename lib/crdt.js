...
class CRDT {
  ...
  insertChar(char, pos) {
    if (pos.line > this.struct.length - 1) {
      this.struct.push([]);
    }
  ...
  findPosition(char) {
    for (let line = 0; line < this.struct.length; line++) {
      ...
      // chars will be equal when looking for position at which to delete; method could be optimized for deletes
      ...
      // char is greater than all chars on this line, return end of line char position
      return { line: line, ch: chars.length }
    }
    // char is greater than all existing chars, return first char position of a new line
    return { line: this.struct.length, ch: 0 }
  }
  ...
  generateChar(val, pos) {
    const posBefore = (this.struct[pos.line] && this.struct[pos.line][pos.ch - 1] && this.struct[pos.line][pos.ch - 1].position) || [];
    const posAfter = (this.struct[pos.line] && this.struct[pos.line][pos.ch] && this.struct[pos.line][pos.ch].position) || [];
