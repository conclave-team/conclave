...
class CRDT {
  ...
  handleLocalInsert(values, pos) {
    ...
    this.controller.broadcastInsertion(chars);
  }

  handleRemoteInsert(chars) {
    let char = chars[0];
    const posStatic = this.findPosition(char);
    ...
    console.log(this.struct);
    this.controller.insertIntoEditor(chars, posStatic, pos, char.siteId);
  }
  ...
  handleRemoteDelete(chars, siteId) {
    chars.forEach(char => {
      ...
      this.removeEmptyLines();
      console.log(this.struct);
      this.controller.deleteFromEditor(chars, pos, siteId);
    });

    // const pos = this.findPosition(chars[0]);
    // let anchor = 0;
    // let runner = 1;
    // let replaceText = '';
    //
    ...
    //   let char = chars[i];
    //   let charToDelete = this.struct[pos.line][pos.ch];
    //
    //   if (char.compareTo(charToDelete) === 0) {
    //     runner++;
    //   } else {
    //     this.struct[pos.line].splice(anchor, runner);
    //
    //   }
    //
    //   if (chars[i - 1].value === '\n') {
    ...
    // const startPos = this.findPosition(startChar);
    // const endPos = this.findPosition(lastChar);
    ...
    //   this.struct[endPos.line].splice(0, endPos.ch);
    //
    //   // single-line deletes
    // } else {
    //   charNum = endPos.ch - startPos.ch;
    ...
    // }
    //
    // this.controller.deleteFromEditor(chars, startPos, endPos, siteId);
  }
