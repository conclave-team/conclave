...
class CRDT {
  ...
  handleLocalInsert(val, index) {
    ...
    this.insertAndUpdate(index, newChar, true);
  ...
  handleRemoteInsert(char) {
    ...
    this.insertAndUpdate(index, char, false);
  }
  ...
  insertAndUpdate(index, char, isLocal) {
    this.insertChar(index, char);
    this.insertText(char.value, index);

    if (isLocal) {
      this.controller.updateEditorLocal();
    } else {
      this.controller.updateEditorRemote('insert', index, char);
    }
  }

  handleLocalDelete(idx) {
    this.vector.increment();
    const deletedChar = this.struct.splice(idx, 1)[0];
    this.deleteText(idx);
    this.controller.updateEditorLocal();
  ...
  handleRemoteDelete(char) {
    ...
    this.controller.updateEditorRemote('delete', index, char);
  }
