...
class CRDT {
  ...
  handleRemoteInsert(char) {
    const pos = this.findCharPosition(char);
...
// only inserts at beginning or end of line; also to be replaced with binary search
  findCharPosition(incomingChar) {
    console.log(this.struct);
    for (let i = 0; i < this.struct.length; i++) {
      let line = this.struct[i];
      let left = 0;
      let right = line.length - 1;

      if (line.length === 0 || incomingChar.compareTo(line[left]) < 0) {
        return { line: 0, ch: 0 };
      } else if (incomingChar.compareTo(line[right]) > 0) {
        return { line: 0, ch: line.length };
      }
    }
  }
