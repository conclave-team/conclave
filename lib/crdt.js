...
class CRDT {
  ...
  handleLocalDelete(startPos, endPos) {
    ...
    if (startPos.line !== endPos.line) {
      ...
      // if newline removed, concat start line with the next line (if it's non-empty)
      ...
      for (line = 0; line < this.struct.length; line++) {
        if (this.struct[line].length === 0) {
          this.struct.splice(line);
          line--;
        }
      }
    } else {
      // for same line mass-delete, just delete chars from startPos.ch to endPos.ch
